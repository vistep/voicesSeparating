...
for filter=1:correlogram.nfilters,
   ...
   for delay=2:(correlogram.ndelays-1)
      ...
      if ((correlogram.data(filter, delay) > correlogram.data(filter, delay+1)) & ...
         ...
         if ((markersize <= 0) | (linewidth <= 0))
            ...
            plot(correlogram.delayaxis(delay), correlogram.freqaxiserb(filter), 'b+', 'Linewidth', linewidth, 'Markersize', markersize);
         end;
      end;
   ...
   if ((markersize <= 0) | (linewidth <= 0))
      ...
      % do plot
      plot(correlogram.delayaxis(bestposition), correlogram.freqaxiserb(filter), 'r*', 'Linewidth', linewidth, 'Markersize', markersize);
   end;
end;
